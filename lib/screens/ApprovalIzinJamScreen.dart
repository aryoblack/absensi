...
class ApprovalIzinJamScreen extends StatefulWidget {
  const ApprovalIzinJamScreen({super.key});

  @override
  State<ApprovalIzinJamScreen> createState() => _ApprovalIzinJamScreenState();
}

class _ApprovalIzinJamScreenState extends State<ApprovalIzinJamScreen> {
  List<dynamic> _data = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    ...
    try {
      final result = await ApiService.fetchPengajuanIzinJam(); // Ganti sesuai endpoint API
      setState(() {
        _data = result;
      });
    } catch (e) {
  ...
  Future<void> _updateStatus(int id, String status) async {
    final success = await ApiService.updateStatusIzinJam(id, status); // Ganti sesuai endpoint API
    if (success) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        foregroundColor: Colors.white,
        title: const Text('Approval Izin Jam'),
      ),
      body: _loading
          ? const Center(child: CircularProgressIndicator())
          : _data.isEmpty
          ? const Center(child: Text('Tidak ada pengajuan izin jam'))
          : ListView.builder(
            ...
            child: ListTile(
              ...
              subtitle: Column(
                ...
                children: [
                  ...
                  Text('Jam: ${izin['jam_mulai']?.toString().substring(0, 5) ?? '-'} - ${izin['jam_selesai']?.toString().substring(0, 5) ?? '-'}'),
